"""views/agent.py — Risk Intelligence Agent tab."""
...
def render():
    ...
    st.markdown(
        ...
        <div style="background:linear-gradient(135deg,#f0fdf4 0%,#dcfce7 100%);
             border:1px solid #bbf7d0; border-radius:12px; padding:1rem 1.4rem; margin-bottom:1.2rem;">
            <p style="margin:0; font-size:0.95rem; color:#166534;">
                🤖 <strong>Risk Intelligence Agent</strong> — Ask a question about any company's risks.
                The agent will automatically score, prioritize, and generate a structured report.
            </p>
        </div>
        """,
    ...
    # ── Configuration ────────────────────────────────────────────────────────
    st.markdown('<div class="section-header">⚙️ Configure Agent</div>', unsafe_allow_html=True)
    ...
    # Optional: compare with prior year
    use_compare = st.checkbox("Include year-over-year comparison", key="agent_use_compare")
    prior_year = None
    if use_compare:
        ...
        if prior_opts:
            ...
            st.caption("No prior year available for this company.")
            use_compare = False
    ...
    for i, q in enumerate(SUGGESTED_QUERIES):
        with cols[i % 3]:
            if st.button(q, key=f"sq_{i}", use_container_width=True):
                st.session_state["agent_query_input"] = q
                st.rerun()
    ...
    user_query = st.text_area(
        ...
        height=80,
        placeholder="e.g. What are the most critical risks and what should we watch for?",
    ...
    if run:
        if not user_query.strip():
            st.error("Please enter a query or select a suggested one.")
            return

        # Load the filing result
        rec = next(
            (r for r in co_recs if r["year"] == year),
            None,
        )
        if not rec:
        ...
        st.session_state["agent_query_input"] = user_query.strip()
        st.rerun()
...
def _display_report(report: dict):
    ...
    st.markdown(
        ...
        <div style="background:#f8f9fb; border:1px solid #e0e3e8; border-radius:12px;
            ...
            <div style="text-align:right;">
                <span style="font-size:0.75rem; color:#6b7280;">Overall Risk Rating</span><br>
                <span style="background:{rc}; color:#fff; padding:4px 14px;
                      border-radius:20px; font-weight:700; font-size:0.9rem;">{overall}</span>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    # Priority matrix metrics
    st.markdown('<div class="section-header">📊 Priority Matrix</div>', unsafe_allow_html=True)
    m1, m2, m3 = st.columns(3)
    m1.metric("🔴 High Priority", pm.get("high", {}).get("count", 0))
    m2.metric("🟡 Medium Priority", pm.get("medium", {}).get("count", 0))
    m3.metric("🟢 Low Priority", pm.get("low", {}).get("count", 0))
    ...
    if high_top:
        ...
        for r in high_top:
            st.markdown(
                f'<div class="card" style="border-left:4px solid #ef4444; padding:0.8rem 1rem; margin-bottom:0.5rem;">'
                f'<div style="display:flex; justify-content:space-between; align-items:flex-start;">'
                f'<span style="font-size:0.9rem; color:#111827; font-weight:500;">{r["title"]}</span>'
                f'<span style="margin-left:1rem; white-space:nowrap;">'
                f'{_priority_badge("High")} '
                f'<span style="font-size:0.78rem; color:#6b7280;">score: {r["score"]}</span>'
                f'</span></div>'
                f'<p style="margin:0.3rem 0 0 0; font-size:0.82rem; color:#6b7280;">{r.get("reasoning","")}</p>'
                f'<p style="margin:0.2rem 0 0 0; font-size:0.78rem; color:#9ca3af;">Category: {r.get("category","")}</p>'
                f'</div>',
                unsafe_allow_html=True,
            )
    ...
    if themes:
        theme_html = " ".join(
            ...
            f'border-radius:20px; font-size:0.82rem; margin-right:4px;">{t}</span>'
            for t in themes
        )
    ...
    if recs:
        ...
        for i, r in enumerate(recs, 1):
            st.markdown(
                ...
                f'<span style="color:#374151;">{r}</span></div>',
                unsafe_allow_html=True,
            )
    ...
    if enriched_risks:
        ...
        for cat_block in enriched_risks:
            ...
            if priority_filter != "All":
                filtered_subs = [s for s in subs if isinstance(s, dict) and s.get("priority") == priority_filter]
            ...
            with st.expander(f"**{cat}** ({len(filtered_subs)} risks)", expanded=False):
                for s in filtered_subs:
                    ...
                    badge = _priority_badge(p)
                    st.markdown(
                        f'<div style="border-left:3px solid {_priority_color(p)}; '
                        f'padding:0.4rem 0.8rem; margin-bottom:0.4rem;">'
                        f'<div style="display:flex; justify-content:space-between;">'
                        f'<span style="font-size:0.88rem; color:#111827;">{s.get("title","")[:150]}</span>'
                        f'<span style="white-space:nowrap; margin-left:0.5rem;">'
                        f'{badge} <span style="font-size:0.75rem; color:#9ca3af;">{score}</span>'
                        f'</span></div>'
                        f'<p style="margin:0.2rem 0 0 0; font-size:0.78rem; color:#9ca3af;">'
                        f'{s.get("reasoning","")}</p>'
                        f'</div>',
                        unsafe_allow_html=True,
                    )
